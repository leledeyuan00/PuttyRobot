...
L_actual = L_len(:,data_begin); % 待求的未知位姿对应的驱动杆长，已知量
L_origin = [sum(L_len(:,data_begin))/3; sum(L_len(:,data_begin))/3; sum(L_len(:,data_begin))/3]; % 数值迭代计算的起始位姿（两平台平行）下对应的驱动杆长，已知量
L_theory = L_origin;
P_origin = [ sum(L_len(:,data_begin))/3; 0; 0 ]; % 基准位姿，自定（P = [ 动平台中心点 Z坐标值， α， β] )
...
% set(gca,'YLim',[-0.4 1.4],'YTick',[-0.4:0.2:1.4]);
