...
para_solu::para_solu(ros::NodeHandle &nh):nh_(nh)
{
    ros_init();
}
...
void para_solu::ros_init()
{
    ...
    joint_info1.sub = nh_.subscribe("/laser_topic", 10, &para_solu::laser_callback1,this);
    #else
    joint_info1.pub = nh_.advertise<std_msgs::Float64>("/parabot/joint1_position_controller/command", 10);
    joint_info1.sub = nh_.subscribe("/parabot/laser1", 10, &para_solu::laser_callback1,this);
    #endif
    joint_info1.stat = 0;
    joint_info1.laser = 0;
    joint_info1.cmd = 0;
    ...
    joint_info2.sub = nh_.subscribe("/parabot/laser2", 10, &para_solu::laser_callback2,this);
    #endif
    joint_info2.stat = 0;
    joint_info2.laser = 0;
    joint_info2.cmd = 0;
    ...
    joint_info3.sub = nh_.subscribe("/parabot/laser3", 10, &para_solu::laser_callback3,this);
    #endif
    joint_info3.stat = 0;
    joint_info3.laser = 0;
    joint_info3.cmd = 0;
...
#ifndef SIMULATE
void para_solu::laser_callback1(const ppr_msgs::laser &msg)
{
...
#else
void para_solu::laser_callback1(const sensor_msgs::LaserScan &msg)
{
    para_motor_[0].laser = msg.ranges[0];
}
void para_solu::laser_callback2(const sensor_msgs::LaserScan &msg)
{
    para_motor_[1].laser = msg.ranges[0];
}
void para_solu::laser_callback3(const sensor_msgs::LaserScan &msg)
{
    para_motor_[2].laser = msg.ranges[0];
}
#endif
void para_solu::pub_msgs(void)
{
...
Eigen::Vector3f para_solu::inverse_solu(Eigen::Matrix3f rotm, float top_z, Eigen::Matrix3f& xzy, Eigen::Vector3f& xyz_v)
{
    /* variable */
...
void para_solu::eul2Rotm(Eigen::Vector3f& euler_ZYX, Eigen::Matrix3f& rotm)
{
    //将欧拉角转变为旋转矩阵
    float tan3;
...
Eigen::Vector3f para_solu::rotm2Eul(Eigen::Vector3f vec_in)
{
    Eigen::Vector3f vec;
    float a,b,c;
...
Eigen::Matrix3f para_solu::normal_vec_rotm(Eigen::Vector3f normal_vec)
{
...
Eigen::Matrix<float, 2, 3> para_solu::get_rot_element(Eigen::Vector3f vec)
{
    Eigen::Matrix<float,2,3> element;
    float x,y,z,a,b,c;
...
Eigen::Vector3f para_solu::get_wall_plat_vec(Eigen::Vector3f laser_dist)
{
...
float para_solu::max_error(Eigen::Vector3f vector)
{
...
void para_solu::calculate(void)
{
    ...
    // ROS_INFO("Wall Eular is : [%f, %f, %f]", pid_error(0), pid_error(1),pid_error(2));
    ...
    // ROS_INFO("temp dist is [%f, %f, %f]", temp_dist(0),temp_dist(1),temp_dist(2));
    ...
    // ROS_INFO("para_motor_ dist is [%f, %f, %f]", para_motor_[0].cmd,para_motor_[1].cmd,para_motor_[2].cmd);
...
Eigen::Vector3f para_solu::get_eular(void)
{
...
{
    ros::init(argc, argv, "para_solution");
    ros::NodeHandle nh;
